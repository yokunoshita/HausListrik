...
- saat baterai turun, app bisa ngomel lewat Windows Speech atau custom WAV
...
- `System.Speech` untuk fallback voice
...
```bash
dotnet restore HausListrik.sln
```
...
Saat `publish`, folder voice pack sekarang ikut disalin ke output build/publish secara otomatis.
...
- Window dan tray sekarang pakai icon brand `Haus Listrik` yang digenerate langsung dari code, jadi belum perlu asset `.ico` terpisah.
- Voice line sekarang punya cooldown supaya penurunan baterai cepat tidak spam suara terus-menerus.
